...
if ($('#search_product').length) {
    let allowAutocomplete = true;

    // Check input and enable/disable autocomplete
    ...
    // Enter key search for numeric input
    $('#search_product').on('keydown', function (e) {
        if (e.key === 'Enter') {
            ...
            if (/^\d+$/.test(term)) {
                ...
                let price_group = $('#price_group').length ? $('#price_group').val() : '';
                let search_fields = [];
                $('.search_fields:checked').each(function (i) {
                    search_fields[i] = $(this).val();
                });

            get_product_suggestion_list(global_p_category_id, 
                global_brand_id, 
                ...
                null,
                term);
                $('#suggestion_page_loader').fadeIn(500);
                ...
                $.getJSON('/products/list', {
                    ...
                    if (data.length === 1) {
                        const item = data[0];
                        const purchase_line_id = null;
                        pos_product_row(item.variation_id, purchase_line_id);

                        const $row = $(`#pos_table tbody tr input.row_variation_id[value="${item.variation_id}"]`).closest('tr');
                        $row.find('input.input_quantity').focus().select();
    ...
    $('#search_product')
        .autocomplete({
            ...
            source: function (request, response) {
                if (!allowAutocomplete) return false;
            ...
            response: function (event, ui) {
                if (ui.content.length === 1) {
                    ...
                    const is_overselling_allowed = $('input#is_overselling_allowed').length > 0;
                    const for_so = $('#sale_type').length && $('#sale_type').val() === 'sales_order';
            ...
            focus: function (event, ui) {
                return ui.item.qty_available > 0;
            },
            select: function (event, ui) {
                const searched_term = $(this).val();
                const is_overselling_allowed = $('input#is_overselling_allowed').length > 0;
                const for_so = $('#sale_type').length && $('#sale_type').val() === 'sales_order';
                const is_draft = $('#status') && ($('#status').val() === 'quotation' || $('#status').val() === 'draft');

                if (ui.item.enable_stock != 1 || ui.item.qty_available > 0 || is_overselling_allowed || for_so || is_draft) {
                    $(this).val(null);

                    const purchase_line_id = ui.item.purchase_line_id && searched_term === ui.item.lot_number ? ui.item.purchase_line_id : null;
                    pos_product_row(ui.item.variation_id, purchase_line_id);

                    const $row = $(`#pos_table tbody tr input.row_variation_id[value="${ui.item.variation_id}"]`).closest('tr');
                    $row.find('input.input_quantity').focus().select();
                }
            }
        })
        .autocomplete('instance')._renderItem = function (ul, item) {
            ...
            if (item.enable_stock == 1 && item.qty_available <= 0 && !is_overselling_allowed && !for_so && !is_draft) {
                let html = `<li class="ui-state-disabled">${item.name}`;
                if (item.type === 'variable') html += `-${item.variation}`;
                const selling_price = item.variation_group_price || item.selling_price;
                html += ` (${item.sub_sku})<br> Price: ${__currency_trans_from_en(selling_price, false, false, __currency_precision, true)} (Out of stock)</li>`;
                return $(html).appendTo(ul);
            } else {
                let html = `<div>${item.name}`;
                if (item.type === 'variable') html += `-${item.variation}`;
                const selling_price = item.variation_group_price || item.selling_price;
                html += ` (${item.sub_sku})<br> Price: ${__currency_trans_from_en(selling_price, false, false, __currency_precision, true)}`;
                if (item.enable_stock == 1) {
                    const qty = __currency_trans_from_en(item.qty_available, false, false, __currency_precision, true);
                    html += ` - ${qty} ${item.unit}`;
                }
                html += '</div>';
    ...
    $('table#pos_table tbody').on('change', 'input.pos_unit_price_inc_tax', function() {
        ...
        var prod_id = $(this).parents('tr').find('.row_variation_id').val();
        var price = $(this).val();
        $.ajax({
            method: 'POST',
            url: '/products/update_price',
            data: { prod_id: prod_id, price: price },
...
// debugger
    ...
    $('button.pos-express-upi').click(function () {
        // debugger
        //Check if product is present or not.
        if ($('table#pos_table tbody').find('.product_row').length <= 0) {
            toastr.warning(LANG.no_products_added);
...
// on click sub category in category drawer
    ...
    $('#pos_table').on('keypress', 'input.pos_quantity', function(e) {
        ...
        if (key == 13 || key == 9) { 
            e.preventDefault();
            var unitPriceInput = $(this).parents('tr').find('input.pos_unit_price_inc_tax');
            unitPriceInput.focus().select();
        }
    });
    $('#pos_table').on('keypress', 'input.pos_unit_price_inc_tax', function(e) {
    var key = e.which;
    if (key == 13 || key ==9) { 
        e.preventDefault();
    ...
    $('#exchange_rate').change(function() {
        ...
        $('span#total_payable').text(__currency_trans_from_en(customRound(shown_total).toFixed(2), false));
    });
...
function get_product_suggestion_list(category_id, brand_id, location_id, url = null, is_enabled_stock = null, repair_model_id = null,term=null) {
...
function pos_total_row() {
    ...
    $('span.price_total').html(__currency_trans_from_en(customRound(price_total).toFixed(2), false));
...
function calculate_billing_details(price_total) {
    ...
    __write_number($('input#final_total_input'), customRound(total_payable_rounded).toFixed(2));
    var curr_exchange_rate = 1;
